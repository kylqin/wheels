...
describe('Compose组合', () => {
  describe('Common accumulate', () => {
    const { compose } = require('../common/accumulate');
    test(compose);
  });

  describe('Functional reduce', () => {
    const { compose } = require('../functional/reduce');
    test(compose);
  });

  describe('Functional reduceRight', () => {
    const { compose } = require('../functional/reduce-right');
    test(compose);
  });

  describe('Functional reduce-promise', () => {
    const { compose } = require('../functional/reduce-promise');
    test(compose);
  });

  describe('OOP link', () => {
    const { compose } = require('../oop/link');
    test(compose);
  });
